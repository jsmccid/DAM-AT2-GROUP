...
# save(test_rat_svdf, file = "./test_rat_svdf")
...
test_xg <- left_join(test_rat_svdf, test_sel, by = c("user_id","item_id"))

test_xg$user_id <- as.numeric(test_xg$user_id)
test_xg$item_id <- as.numeric(test_xg$item_id)
test_xg <- test_xg %>% 
  select(-user_item)

test_xg_1h <- dummyVars(~., data = test_xg, fullRank = TRUE)
test_xg_1h <- predict(test_xg_1h, newdata = test_xg)
...
control_dart <- trainControl(method = "adaptive_cv",
  ...
  # save(dart_mod, file = "./models/dart_collab.RData")
  
  dart_mod
  
  out_xg_xvdf <- test_xg
  
out_xg_xvdf$rating <- predict(dart_mod, test_xg, ntree_limit=num_round)

out_xg_xvdf$user_item <- paste(out_xg_xvdf$user_id, out_xg_xvdf$item_id, sep="_")
preds <- out_xg_xvdf %>% 
  ungroup() %>% 
 ...
 write_csv(preds, "./predictions/xg_dart_svdf2.csv")
