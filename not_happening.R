...
# Kailash cosine similarity funtion
...
# move_clus <- kmeans(movies_matrix,100,nrow(movies)) deprecated

movies_matrix_norm <- movies_matrix

#normalize col values
movies_matrix_norm[,-c(2:20,23:35)] <- scale(movies_matrix_norm[,-c(2:20,23:35)])

#scale between 0 and 1
movies_matrix_norm[,-c(2:20,23:35)] <- apply(movies_matrix_norm[,-c(2:20,23:35)], 2, rescale)

cosine_movie_clus <- akmeans(movies_matrix_norm,  min.k = 5, max.k = nrow(movies_matrix_norm),  iter.max = 1000, d.metric = 2)
#convereged at k=128

## using akmeans for now, too long to calc otherwise
# clusplot(movies_matrix, move_clus$cluster, color=T, shade=T, labels=2, lines=0)
# clusplot(movies_matrix_norm, cosine_movie_clus$cluster, color=T, shade=T, labels=2, lines=0)
# 
 cs <- cosineSim(movies_matrix_norm)
 cd <- 1-cs
# 
# kmeanspp(cd, k = i, start = "random", iter.max = 100, nstart = 10, ...)
 kmeans(cd,centers=128,nstart=25)
# 
 wss <- 2:(nrow(movies_matrix_norm)-1670)
 foreach (i = 2:(nrow(movies_matrix_norm)-1670)) %dopar% { 
   wss[i] <- sum(kmeans(cd,centers=i,nstart=25)$withinss)
   }
 for (i in 2:(nrow(movies_matrix_norm)-1680)) wss[i] <- sum(kmeans(cd,centers=i,nstart=25)$withinss)
